...
import { defaultSize, estimateMatch, MAX_PLAYERS, MIN_PLAYERS, PLAYERS, SIZE_IDS, SIZES, type SizeId } from "@/lib/frontline/config";
import type { MatchEvent } from "@/lib/frontline/match";
import { MatchView } from "./MatchView";
import { beatMs, FrontlineModel } from "./model";
...
export default function FrontlineLobby({ providers }: { providers: ProviderMeta[] }) {
  ...
  const [model, setModel] = useState<FrontlineModel | null>(null);
  const [, setVersion] = useState(0);
  const abortRef = useRef<AbortController | null>(null);
  const queue = useRef<MatchEvent[]>([]);
  const pumping = useRef<Promise<void> | null>(null);
  const modelRef = useRef<FrontlineModel | null>(null);
    ...
    <>
      <section className="panel setup" aria-label="Match setup">
            ...
            >
              {SIZE_IDS.map((id) => (
                <option key={id} value={id}>
                  {SIZES[id].label} · {SIZES[id].rounds} rounds
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="estimate">
          {provider === "demo"
            ? `Scripted bots in every seat, free · up to ${SIZES[size].rounds} rounds`
            : `${players} JEV agents on one key · up to ${est.calls} calls · about ${tokens(est.tokens)} tokens at most`}
        </div>
      ...
      {model && <MatchView model={model} version={model.applied} status={status} />}
    </>
  );
}
