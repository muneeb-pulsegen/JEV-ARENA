...
import { beatMs, DominionModel, type StartEvent } from "../model";
...
function modelAt(start: StartEvent, events: MatchEvent[], n: number) {
  const m = new DominionModel(start);
  for (let k = 0; k < n; k++) m.apply(events[k]);
  return m;
}

/** Plays a saved match back through the same view as a live match, turn by turn. */
export default function DominionReplay({ log }: { log: MatchEvent[] }) {
  const start = log.find((e): e is StartEvent => e.type === "start")!;
  const events = log.filter((e) => e.type !== "start" && e.type !== "retry");
  // Index just past each turn's last event, so the slider can jump to the end of any turn.
  const turnEnds = events.flatMap((e, k) => (e.type === "turn" ? [k + 1] : []));
  const total = turnEnds.length;

  const [cursor, setCursor] = useState(0);
  const [playing, setPlaying] = useState(true);
  const [speed, setSpeed] = useState<(typeof SPEEDS)[number]>(1);
  const [model, setModel] = useState(() => new DominionModel(start));
  ...
  const turn = turnEnds.filter((k) => k <= cursor).length;
      ...
      <div className="panel timeline">
        <input
          ...
          onChange={(e) => {
            ...
            setCursor(r === 0 ? 0 : r === total ? events.length : turnEnds[r - 1]);
          }}
          aria-label="Turn"
        />
        <span className="hint">turn {turn} / {total}</span>
      </div>
      <DominionView
        model={model}
        version={version}
